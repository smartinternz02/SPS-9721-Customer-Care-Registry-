...
app.config['MYSQL_USER'] = "KWrIEvMVDM"
app.config['MYSQL_PASSWORD'] = "3Mmfd2Y457"
app.config['MYSQL_DB'] = "KWrIEvMVDM"
...
def complainregistry():
    ...
    if request.method =="POST":
        ...
        complain_description = request.form["complain description"]
        cursor = mysql.connection.cursor()
        cursor.execute('SELECT * FROM signup WHERE name = % s', (name,))
        account = cursor.fetchone()
        if not account:
          ...
          cursor = mysql.connection.cursor()
          cursor.execute('SELECT complainId FROM complain_registry WHERE name= %s', (name,))
...
def adminlogin():
    ...
    if request.method == 'POST' :
        ...
        password = request.form['password']
        if adminname == 'Nidhi' and password == 'abc123':
            session['loggedin'] = True
           # session['id'] = account[0]
           # userid=  account[0]
           # session['name'] = account[1]
            return render_template('daashboard.html')
        else:
          msg = 'Incorrect username / password !'
